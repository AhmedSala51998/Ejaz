...
Route::group(
    ...
    // route لكل فرع
    /*Route::get('{branch}', [\App\Http\Controllers\Frontend\Home\HomeFrontController::class,'index'])
        ->where('branch','yanbu|riyadh|jeddah')
        ->name('branch.home');*/

    // route للـ "/" بحيث يكون default ويعطي parameter
    /*Route::get('/', function () {
        return redirect()->route('branch.home', ['branch' => session('branch')]);
    })->name('home');*/
    Route::get('/', [\App\Http\Controllers\Frontend\Home\HomeFrontController::class,'showLanding'])->name('home');
    ...
    Route::group(['prefix' => '{branch}', 'where' => ['branch' => 'yanbu|riyadh|jeddah']], function() {

        Route::post('contact-us', [\App\Http\Controllers\Frontend\Home\HomeFrontController::class,'contact_us_action'])
            ->name('front.contact_us_action');

        Route::get('register/{id?}', [\App\Http\Controllers\Frontend\Auth\RegisterFrontController::class,'register_view'])
            ->name('register');
        Route::post('checkPhoneToSendOtp', [\App\Http\Controllers\Frontend\Auth\RegisterFrontController::class,'check_phone_to_send_otp'])
            ->name('checkPhoneToSendOtp');
        Route::post('registerAction', [\App\Http\Controllers\Frontend\Auth\RegisterFrontController::class,'register_action'])
        ...
        Route::get('forget-password', [\App\Http\Controllers\Frontend\Auth\ForgetPasswordFrontController::class,'forget_password_view'])
            ->name('auth.forget_password_view');
        Route::post('forget-password-action', [\App\Http\Controllers\Frontend\Auth\ForgetPasswordFrontController::class,'forget_password_action'])
            ->name('auth.forget_password_action');
        Route::get('forget-email-sent-successfully', [\App\Http\Controllers\Frontend\Auth\ForgetPasswordFrontController::class,'forget_password_email_successfully_sent'])
            ->name('auth.forget-email-sent-successfully');

        Route::get('reset-password', [\App\Http\Controllers\Frontend\Auth\ResetPasswordFrontController::class,'reset_password_view'])
            ->name('auth.reset_password_view');
        Route::post('reset-password-action', [\App\Http\Controllers\Frontend\Auth\ResetPasswordFrontController::class,'reset_password_action'])
            ->name('auth.reset_password_action');

        Route::get('track-order', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'track_order_view'])
            ->name('track_order_view');
        Route::post('post-track-order', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'track_order'])
            ->name('track_order');
        Route::get('order_details/{id}', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'order_details'])
            ->name('order_details');

        Route::get('countries', [\App\Http\Controllers\Frontend\CountriesController::class,'index'])
            ->name('frontend.show.countries');
        Route::get('ourStaff', [\App\Http\Controllers\Frontend\OurStaffController::class,'index'])
            ->name('frontend.show.ourStaff');
        Route::get('about', [\App\Http\Controllers\Frontend\AbouUsController::class,'index'])
            ->name('frontend.aboutUs');

        Route::get('completeTheRecruitmentRequest/{id}', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'completeTheRecruitmentRequest'])
            ->name('front.completeTheRecruitmentRequest');
        Route::post('cancel-reservation/{id}', [\App\Http\Controllers\Admin\CRUD\AdminOrderController::class, 'autoCancelReservation']);
    ...
    Route::get('{branch}/all-workers/{id?}', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'showAllWorkers'])
    ->where('branch','yanbu|riyadh|jeddah')
    ->name('all-workers');
    ...
    Route::get('{branch}/worker/{id}', [\App\Http\Controllers\Frontend\Worker\WorkerFrontController::class,'show'])
    ->where('branch','yanbu|riyadh|jeddah')
    ->name('frontend.show.worker');
    ...
    Route::get('{branch}/transferService', [\App\Http\Controllers\Frontend\TransferServices\TransferServicesFrontController::class, 'transferService'])
        ->where('branch', 'yanbu|riyadh|jeddah')
        ->name('transferService');

    Route::get('{branch}/services-single', [\App\Http\Controllers\Frontend\Rental\RentalController::class, 'rental'])
        ->where('branch', 'yanbu|riyadh|jeddah')
